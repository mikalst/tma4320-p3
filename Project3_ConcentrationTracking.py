...
"""
def plotConcentrationOnMapTest():
    ...
    # Set how time interval
    start_time = 0
    end_times = 24*10#[48*n for n in range(1, 6)]
    ...
    # Current and final time is set
    time_current = np.datetime64('2017-02-01T00:00:00') + np.timedelta64(start_time, 'h')
    time_final = time_current + np.timedelta64(end_times, 'h')

    # Particle positions in final time is calculated
    X1 = P_Uf.particleTrajectory(X0, time_current, h, time_final, velocityField, P_Int.rk2, boolReturnOnlyEnd=True)

    # Set grid cell size of approximately 800
    bins = max(int(max(X1[0, :]) - min(X1[0, :]))/800,
               int(max(X1[1, :]) - min(X1[1, :]))/800)

    # Calculate 2d histogram of particlepositions with a resolution of ~800m/pixel
    C, x, y = np.histogram2d(X1[0, :], X1[1, :], bins)
